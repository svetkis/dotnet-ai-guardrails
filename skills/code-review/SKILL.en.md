...
- **EF Core Write-Path**: No `.AsNoTracking()` on write operations. Exception: raw SQL (`FromSqlRaw`, `ExecuteSqlRaw`) and bulk update API (`ExecuteUpdateAsync`) — Change Tracker does not track them anyway.
