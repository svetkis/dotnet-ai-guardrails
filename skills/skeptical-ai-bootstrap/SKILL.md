...
- Копировать структуру папок `dotnet-skeptical-ai` в целевой репо (НЕ создавать в корне целевого проекта: `rules/`, `skills/`, `tests/patterns/`. Исключение: `.kimi/skills/` для markdown-скиллов — это нормально)
...
**Проверка окружения (делай ЭТО ПЕРВЫМ ДЕЛОМ):**
1. Посмотри на путь текущей рабочей директории. Содержит ли он `dotnet-skeptical-ai`?
   - ДА → Ты в репозитории методологии. Твоя задача — читать артефакты и объяснять их. НЕ модифицируй этот репо без явной просьбы.
   - НЕТ → Переходи к шагу 2.
2. Есть ли в текущей директории (или непосредственных поддиректориях) файлы `.sln` или `.csproj`?
   - ДА → Ты в целевом .NET-проекте. Сканируй его, оценивай, генерируй отчёты. НЕ создавай демо-код.
   - НЕТ → Спроси пользователя: "Какой .NET-проект нужно оценить? Укажите путь."
...
> **Перед началом:** Выполни проверку окружения из раздела [🚨 CRITICAL: Zero Implementation Rule](#-critical-zero-implementation-rule) выше. Убедись, что ты в целевом проекте, а не в репозитории методологии.
...
> **Перед использованием прочитай:** `docs/agents/BOOTSTRAP-PROTOCOL.md` в репозитории `dotnet-skeptical-ai` — правила поведения агента при онбординге.
