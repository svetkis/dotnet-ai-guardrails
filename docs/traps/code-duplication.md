...
Агент-ревьюер проверяет: если в PR добавлена валидация/расчёт — есть ли похожая логика в других сервисах? См. `skills/code-review/CHECKLIST.md`.

### 3. Tech Debt Audit: Semantic Duplication по кодбазе
Агент (и regex) не способны распознать **семантическое** дублирование по всей кодбазе. Это задача персоно-аудита техлида, который запускается раз в спринт. См. `skills/tech-debt-audit/SKILL.md`.
